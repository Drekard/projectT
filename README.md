...
**Как протестировать P2P на одном компьютере?**

⚠️ **Важно:** У каждого узла должен быть **уникальный PeerID**. 
Для этого нужно использовать разные базы данных.
...
- Создаст две копии базы данных (для двух узлов)
- Запустит два окна PowerShell на разных портах
- Каждый узел будет иметь уникальный PeerID

**Способ 2: Вручную**
```powershell
# Окно 1 - первый узел (порт 8080, своя БД)
$env:PROJECTT_P2P_PORT=8080
$env:PROJECTT_DB_PATH=node1.db
.\projectT.exe

# Окно 2 - второй узел (порт 8081, своя БД)
$env:PROJECTT_P2P_PORT=8081
$env:PROJECTT_DB_PATH=node2.db
.\projectT.exe
```

**Далее:**
1. Откройте настройки P2P в первом окне
2. Скопируйте адрес (он будет уникальным)
3. Добавьте как контакт во втором окне
4. Проверьте подключение во вкладке "Сеть"
