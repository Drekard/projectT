...
│   │   │
│   │   ├── p2p/                         # 🆕 СОЗДАТЬ
│   │   │   ├── network/                 # 🆕 Пакет P2P сети
│   │   │   │   ├── network.go           # Ядро libp2p: создание хоста, запуск/остановка
│   │   │   │   ├── profile.go           # Управление профилем: загрузка, создание, сохранение
│   │   │   │   └── keys.go              # Работа с ключами: генерация, шифрование
│   │   │   ├── address_tools.go         # Экспорт/импорт адресов, форматирование
│   │   │   ├── peer_helper.go           # 🆕 Режим помощника: хранение адресов пиров
...
## 🤖 Режим помощника (Helper Mode)

**Режим помощника** — механизм P2P подключения без использования сторонних серверов, STUN или DHT.

### Как это работает

Вместо использования центрального сервера для обмена адресами, один из узлов выступает в роли **"помощника"** — хранит и передаёт адреса других пиров.

```
Сценарий подключения:

1. Пользователь Б (помощник)
   - Запускается в режиме "помощника"
   - Экспортирует свой адрес: projectt://QmBob@/ip4/192.168.1.5/tcp/4001/p2p/QmBob
   - Передаёт адрес А (через QR-код, буфер обмена, мессенджер)

2. Пользователь А (клиент)
   - Импортирует адрес Б
   - Подключается напрямую к Б
```

### Преимущества

| Критерий | Режим помощника | STUN/сервер |
|----------|-----------------|-------------|
| Сервер | Не нужен | Нужен отдельный |
| Доверие | Прямой обмен | Сервер видит все адреса |
| Сложность | 1 компонент | 2+ компонента |
| Работа без интернета | ✅ Да | ❌ Нет |

### API

```go
// Экспорт своего адреса
addr, err := p2pNetwork.GetPeerAddress()

// Импорт адреса и добавление в контакты
err = p2pNetwork.ImportPeerAddress(addrStr)

// Подключение к пиру
err = p2pNetwork.ConnectToPeer(ctx, addrStr)

// Зарегистрировать свой адрес у удалённого помощника
err = p2pNetwork.HelperRegister(peerID, address)

// Запросить адрес пира у помощника
data, found := p2pNetwork.HelperAsk(targetPeerID)
```

### Документация

См. **[HELPER_MODE_GUIDE.md](HELPER_MODE_GUIDE.md)** — полное руководство по использованию режима помощника.
