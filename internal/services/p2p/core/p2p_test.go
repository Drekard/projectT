// Package core содержит тесты для P2P сети
package core
...
import (
	...
	p2paddress "projectT/internal/services/p2p/address"
	p2pconn "projectT/internal/services/p2p/connection"
	p2pdiscovery "projectT/internal/services/p2p/discovery"
)
...
func TestP2PNetwork_ConnectionService(t *testing.T) {
	t.Run("создание сервиса соединений", func(t *testing.T) {
		...
		service := p2pconn.NewService(h, cfg)
	...
	t.Run("получение статуса подключения", func(t *testing.T) {
		...
		cfg := p2p.DefaultConfig()
		service := p2pconn.NewService(h2, cfg)
		err := service.Start()
		require.NoError(t, err)
		...
		assert.Equal(t, p2pconn.StatusUnknown, status)
		...
		status = service.GetConnectionStatus(h1.ID())
		assert.Equal(t, p2pconn.StatusConnected, status)
	})
}
...
func TestP2PNetwork_DiscoveryService(t *testing.T) {
	t.Run("создание сервиса обнаружения", func(t *testing.T) {
		...
		service := p2pdiscovery.NewDiscoveryService(h, discovery, cfg)
		require.NotNil(t, service)
...
func TestP2PNetwork_Integration(t *testing.T) {
	t.Run("полный цикл: создание, подключение, обнаружение", func(t *testing.T) {
		...
		service1 := p2pdiscovery.NewDiscoveryService(host1, disc1, cfg)
		service2 := p2pdiscovery.NewDiscoveryService(host2, disc2, cfg)
