...
func (d *Dialer) dialOne(addr *PeerAddress) DialResult {
	...
	if err != nil {
		log.Printf("[Autodial] ❌ Подключение к %s (%s): %v", addr.PeerID[:8], addr.AddressType, err)
	...
	log.Printf("[Autodial] ✅ Подключение к %s (%s) успешно", addr.PeerID[:8], addr.AddressType)
	d.mu.Lock()
	d.connectedCount++
	d.mu.Unlock()
...
func (d *Dialer) attemptReconnect(peerID peer.ID) {
	...
	if err != nil {
		log.Printf("[Autodial] Переподключение к %s не удалось: %v", peerID.String()[:8], err)
		d.reconnectQueue = append(d.reconnectQueue, peerID)
	} else {
		log.Printf("[Autodial] Переподключение к %s успешно", peerID.String()[:8])
