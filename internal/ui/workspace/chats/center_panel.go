...
func (ui *UI) createAddContactSection() *fyne.Container {
	...
	ui.addressEntry.MultiLine = true
...
func (ui *UI) createP2PSettingsSection() *fyne.Container {
	...
	stunRow := container.NewHBox(stunLabel, ui.stunServerEntry)
...
func (ui *UI) loadContactsList() {
	...
	// Загружаем контакты из P2P сервиса
	if ui.p2pUI != nil {
		contacts := ui.p2pUI.GetAllContacts()

		for _, contact := range contacts {
			status := widgets.StatusOffline
			if contact.IsConnected {
				status = widgets.StatusOnline
			}

			contactItem := widgets.NewContactItem(&models.Contact{
				Username:  contact.Username,
				PeerID:    contact.PeerID,
				Multiaddr: contact.Address,
				Status:    contact.Status,
			}, 0, status, func() {
				ui.selectChat(&models.Contact{
					Username:  contact.Username,
					PeerID:    contact.PeerID,
					Multiaddr: contact.Address,
				})
		...
		// Тестовые контакты если P2P не инициализирован
