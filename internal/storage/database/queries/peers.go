...
func GetP2PProfile() (*models.P2PProfile, error) {
	row := database.DB.QueryRow(`
		SELECT id, peer_id, private_key, public_key, username, status, listen_addrs, created_at, updated_at
		FROM p2p_profile
		WHERE id = 1
	`)
...
func CreateP2PProfile(profile *models.P2PProfile) error {
	_, err := database.DB.Exec(`
		INSERT INTO p2p_profile (id, peer_id, private_key, public_key, username, status, listen_addrs, created_at, updated_at)
		VALUES (1, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
	`, profile.PeerID, profile.PrivateKey, profile.PublicKey, profile.Username, profile.Status, profile.ListenAddrs)
	return err
}
...
func UpdateP2PProfile(profile *models.P2PProfile) error {
	_, err := database.DB.Exec(`
		UPDATE p2p_profile
		SET peer_id = ?, private_key = ?, public_key = ?, username = ?, status = ?, listen_addrs = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = 1
	`, profile.PeerID, profile.PrivateKey, profile.PublicKey, profile.Username, profile.Status, profile.ListenAddrs)
	return err
}
